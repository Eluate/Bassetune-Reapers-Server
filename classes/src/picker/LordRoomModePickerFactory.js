...
function LordRoomModePickerFactory(place, pickStrategy) {
    this.place = place;
    ...
    this.priv_createPicker = function() {
        ...
        if (selectedCard === "EAST") {
            ...
            barycenterRow = vertex.rowIndex() + Math.floor(height / 2);
            barycenterCol = vertex.columnIndex();
            type = "vertical";
        } else if (selectedCard === "SOUTH") {
            var vertex = this.place.bottomLeftVertex();
            barycenterRow = vertex.rowIndex();
            barycenterCol = vertex.columnIndex() + Math.floor(width / 2);
            type = "horizontal";
...
module.exports = LordRoomModePickerFactory;
