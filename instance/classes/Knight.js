...
Knight.prototype.ChangeEquipped = function (data, slotID, target) {
    ...
    if (target == 0) {
        ...
        if (item1 != null && item2 != null) {
            ...
            data.io.emit(EventEnum.output.knight.END_CHANGE_EQUIPPED, {
                "a": item1[1],
                "b": item2[1],
                "p": this.character.owner
            });
        }
        return;
    }
    ...
    for (var i = 0, slotLength = slots.length; i < slotLength; i++) {
        ...
        if (target == 2 || target == 3 || target == 9) {
            ...
            data.io.to(data.matchID).emit(EventEnum.output.knight.START_CHANGE_EQUIPPED, {
                "i": slot[1],
                "t": target,
                "p": this.character.owner
            });
            ...
            setTimeout(function () {
                ...
                data.io.to(data.matchID).emit(EventEnum.output.knight.END_CHANGE_EQUIPPED, {
                    "i": slot[1],
                    "t": target,
                    "p": self.character.owner
                });
            }, 3000);
        } else if (target == 4) {
            // Armor
